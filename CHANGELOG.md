...
- **ElevenLabs TTS provider (optional `[elevenlabs]` extra)** — voicemode can now use ElevenLabs voices alongside Kokoro / OpenAI. ElevenLabs is the first TTS provider that is not OpenAI-compatible: it gets a dedicated dispatch path in `simple_tts_failover` and uses the official `AsyncElevenLabs` SDK. Setup: `pip install voice-mode[elevenlabs]`, set `ELEVENLABS_API_KEY`, prepend `https://api.elevenlabs.io/v1` to `VOICEMODE_TTS_BASE_URLS`. New CLI group: `voicemode elevenlabs {list-voices,set-api-key,test}`. Cross-provider failover is supported with best-effort voice-ID mapping (e.g. ElevenLabs *Rachel* ↔ OpenAI *nova* ↔ Kokoro *af_sky*).
